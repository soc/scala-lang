...
Scala supports variance annotations of type parameters of [generic classes](generic-classes). Variance annotations may be added when a class is defined (declaration-site variance), whereas in Java variance annotations are given by clients when a class abstraction is used (use-site variance).

In the page about [generic classes](generic-classes) an example for a mutable stack was given. We explained that the type defined by the class `Stack[T]` is subject to invariant subtyping regarding the type parameter. This can restrict the reuse of the class abstraction. We now derive a functional (i.e. immutable) implementation for stacks which does not have this restriction. Please note that this is an advanced example which combines the use of [polymorphic methods](polymorphic-methods), [lower type bounds](lower-bounds), and covariant type parameter annotations in a non-trivial fashion. Furthermore we make use of [inner classes](inner-classes) to chain the stack elements without explicit links.
